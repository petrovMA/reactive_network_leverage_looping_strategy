...
async function main() {
    ...
    // Minimal ABI for pause functionality (from AbstractPausableReactive)
    ...
    console.log("⏸️  Pausing RSC (unsubscribing from all events)...\n");
    console.log(`RSC Address: ${RSC_ADDRESS}`);
    console.log(`Signer Address: ${signer.address}\n`);

    try {
        // Call pause() directly - it will revert with "Unauthorized" if not owner
        // or "Already paused" if already paused
        console.log("⏳ Calling pause() to unsubscribe from all events...");
        const tx = await rsc.pause();
        ...
        console.log("\n====================================================");
        console.log("✅ RSC paused successfully!");
        console.log("   All event subscriptions have been removed.");
        console.log("   Use resume() to re-subscribe to events.");
        console.log("====================================================");
    } catch (error: any) {
        console.log("\n❌ Failed to pause RSC:");
        if (error.reason) {
